...
public class BrokenLineChartView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            pts2[i * 4 + 3] = y2;
            if (i < 4) {
                canvas.drawText(String.valueOf(i * 2 + 2), x1 - 35, y1 + 10, getPaint());
            } else {
                canvas.drawText(String.valueOf(i * 2 + 2), x1 - 45, y1 + 10, getPaint());
            }
        }
        // 计算位置
    ...
    private void initXY(Canvas canvas) {
        ...
        for (int i = 0; i < leftrightlines; i++) {
            double t=0.0;
            if(score.get(i)>12){
                t = 12 * (YScale - YSPace)/10.0;
            }else
                t = score.get(i) * (YScale - YSPace)/10.0;
